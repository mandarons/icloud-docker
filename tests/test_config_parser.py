...
class TestConfigParser(unittest.TestCase):
    ...
    def test_get_sync_interval_valids(self):
        ...
        self.assertEqual(config['app']['sync_interval'], config_parser.get_sync_interval(config=config))
    ...
    def test_prepare_drive_destination_valids(self):
        ...
        self.assertEqual(os.path.abspath(os.path.join(config['app']['root'],config['drive']['destination'])), actual)
        ...
        actual = config_parser.prepare_drive_destination(config=config)
        self.assertEqual(os.path.abspath(os.path.join(constants.DEFAULT_ROOT_DESTINATION,
        constants.DEFAULT_DRIVE_DESTINATION)), actual)
        self.assertTrue(os.path.exists(actual))
        self.assertTrue(os.path.isdir(actual))
        os.rmdir(actual)
    ...
    def test_get_username_valids(self):
        config = config_parser.read_config()
        # Given username
        self.assertEqual(config['app']['credentials']['username'], config_parser.get_username(config=config))
    ...
    def test_get_verbose_valids(self):
        config = config_parser.read_config()
        self.assertEqual(config['app']['verbose'],config_parser.get_verbose(config=config))
    ...
    def test_get_smtp_password_valids(self):
        ...
        self.assertEqual(config['app']['smtp']['password'], config_parser.get_smtp_password(config=config))
